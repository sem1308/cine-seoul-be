...
//    List<Schedule> findByMovieNum(@Param("movieNum") String movieNum);

    // - 영화 제목으로 조회
//    @Query("select s from Schedule s where s.movie.movieName = :movieName")
//    List<Schedule> findByMovieName(@Param("movieName") String movieName);

    // 영화(+날짜)별 상영 일정 조회
    // @Query("select s from Schedule s where s.movie.movieName = :movieName and s.schedTime between :start and :end")
//    List<Schedule> findByMovieNameAndDateBetween(LocalDateTime start, LocalDateTime end, @Param("movieName") String movieName);

    // @Query("select s from Schedule s where s.movie.movieNum = :movieNum and s.schedTime between :start and :end")
//    List<Schedule> findByMovieNumAndDateBetween(LocalDateTime start, LocalDateTime end, @Param("movieNum") String movieNum);
